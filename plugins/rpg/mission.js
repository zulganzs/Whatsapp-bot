...
async function handler(m, { conn, args, text , usedPrefix, command }) {
  ...
  try {
    ...
    if (user.health == 0 || user.stamina == 0) return m.reply(`your stamina/healt is less than 100`)

    if(typeof user.lastmisi != "number") db.data.users[m.sender].lastmisi = 0
    if(typeof user.exp != "number") db.data.users[m.sender].exp = 0
    if(typeof user.crystal != "number") db.data.users[m.sender].crystal = 0
