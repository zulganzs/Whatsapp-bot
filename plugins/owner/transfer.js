...
`.trim()
	...
	if (isNaN(args[1])) throw `Masukkan nominal !`
...
`.trim()
	let c = pauthor
	conn.reply(m.chat, confirm+`\n\nketik *y* untuk ya\nketik *n* untuk tidak`, m, { mentions: [who] })
...
handler.before = async m => {
	...
	if (/y(es)?/g.test(m.text.toLowerCase())) {
		...
		if (count.toString().includes('-')) {
			...
			m.reply(`Succes transfer *${count}* ${rpg.emoticon(type)}${type}${special(type)} to *@${(to || '').replace(/@s\.whatsapp\.net/g, '')}*`, null, { mentions: [to] })
		}
		clearTimeout(timeout)
		delete confirmation[sender]
	}
