let handler = async (m, { conn, usedPrefix, command }) => {
	try {
		...
		await conn.sendMsg(m.chat, { video: fimgb, caption: `_Random Story Anime_` }, { quoted: m })
	} catch (e) {
		console.log(e)
		try {
			await conn.sendMsg(m.chat, { video: { url: dir[Math.floor(Math.random() * dir.length)] }, caption: `_Random Story Anime_` }, { quoted: m })
		} catch (e) {
			console.log(e)
